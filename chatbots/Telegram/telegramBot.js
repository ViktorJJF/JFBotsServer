...
async function sendToDialogFlow(senderID, messageText) {
    ...
    let result = await dialogflow.sendToDialogFlow(senderID, messageText, "TELEGRAM");
...
async function handleCardMessages(messages, senderID) {
    console.log("se recibio esto en handleCardMessages: ", JSON.stringify(messages, null, " "));
...
async function sendGenericMessage(senderID, element) {
    ...
    await sendButtons(senderID, "<b>" + element.title + "</b>" + "\n" + element.subtitle, element.buttons)
}
...
let sendTextMessage = (senderID, message) => {
    if (message.includes("{first_name}") || message.includes("{{last_name}}")) {
        ...
        message = message.replace("{first_name}", userData.first_name).replace("{{last_name}}", userData.last_name);
    }
