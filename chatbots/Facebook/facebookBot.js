...
async function receivedMessage(event) {
    ...
    if (messageText) {
        ...
        let result = await dialogflow.sendToDialogFlow(senderID, messageText, "FACEBOOK");
...
async function handleQuickReply(senderID, quickReply, messageId) {
    ...
    sendTypingOn(senderID);
    let result = await dialogflow.sendToDialogFlow(senderID, quickReplyPayload, "FACEBOOK");
    handleDialogFlowResponse(senderID, result);
}
...
function handleCardMessages(messages, sender) {
    console.log("se enviaran estos mensajes card: ", JSON.stringify(messages, null, " "));
    let elements = [];
...
async function handleMessages(messages, sender) {
    ...
    for (var i = 0; i < messages.length; i++) {
        console.log("mensaje: ", i, messages[i]);
...
async function sendTextMessage(recipientId, text) {
    if (text.includes("{first_name}") || text.includes("{{last_name}}")) {
        ...
        text = text.replace("{first_name}", userData.first_name).replace("{{last_name}}", userData.last_name);
    }
    var messageData = {
        recipient: {
            id: recipientId
